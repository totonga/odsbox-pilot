...
def con_i():
    ...
    from odsbox import ConI  # type: ignore[import-untyped]
...
class TestDemoServerQueries:
    def test_query_units_returns_dataframe(self, con_i) -> None:
    ...
    def test_query_units_with_attributes(self, con_i) -> None:
        df = con_i.query(
            {
                "AoUnit": {},
    ...
    def test_query_with_row_limit(self, con_i) -> None:
        df = con_i.query({"AoUnit": {}, "$options": {"$rowlimit": 3}})
        assert len(df) <= 3

    def test_query_unit_by_name(self, con_i) -> None:
